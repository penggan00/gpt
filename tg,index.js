...
class Gemini {
  ...
  // 修复：使用正确的 Gemini 请求构建器
  request = async (params, context, onStream) => {
    const { prompt, messages } = params;
    const { base, key, model, extraParams } = this.fieldGetter(context);
    ...
    // 构建 Gemini 格式的请求体
    const contents = await this.formatGeminiMessages(prompt, messages);
    
    const body = {
      ...extraParams || {},
      contents,
      generationConfig: {
        maxOutputTokens: ENV.MAX_TOKEN_LENGTH > 0 ? ENV.MAX_TOKEN_LENGTH : 2048
      }
    };
    
    const options = {
      contentExtractor: (data) => data?.candidates?.[0]?.content?.parts?.[0]?.text,
      fullContentExtractor: (data) => data?.candidates?.[0]?.content?.parts?.[0]?.text,
      errorExtractor: (data) => data?.error?.message
    };
    
    return convertStringToResponseMessages(
      requestChatCompletions(url, header, body, onStream, options)
    );
  };
  
  // Gemini 消息格式转换
  async formatGeminiMessages(prompt, messages) {
    const contents = [];
